...
namespace RAML.Parser.Tests
{
    ...
    public class MoviesTests
    {
        ...
        public void Post_Operation_Security()
        {
            ...
            Assert.AreEqual("OAuth 2.0", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().Name);
            Assert.AreEqual("https://localhost:8081/oauth/authorize", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Settings.AuthorizationUri);
            Assert.AreEqual("https://localhost:8081/oauth/access_token", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Settings.AccessTokenUri);
            Assert.AreEqual("authorization_code", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Settings.AuthorizationGrants.First());
            Assert.AreEqual("read", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Settings.Scopes.First().Name);
            Assert.AreEqual("write", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Settings.Scopes.Last().Name);
            Assert.AreEqual("access_token", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.QueryParameters.First().Name);
            Assert.AreEqual("Authorization", model.WebApi.EndPoints.First(e => e.Path == "/movies").Operations.First(o => o.Method == "post").Security.First().Schemes.First().SecurityScheme.Headers.First().Name);
        }
