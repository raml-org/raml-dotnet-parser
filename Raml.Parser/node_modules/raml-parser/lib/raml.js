(function() {
  ...
  this.FileReader = (function() {
    ...
    */
    ...
    FileReader.prototype.readFileAsync = function(file, path) {
      var targerUrl;
      ...
      if (targerUrl.protocol != null) {
        ...
        if (typeof window !== "undefined" && window !== null) {
          ...
          return this.fetchLocalFileAsync(file, path);
        }
      }
    };
    ...
    */
    ...
    FileReader.prototype.fetchLocalFileAsync = function(file, path) {
      var deferred,
        _this = this;
      deferred = this.q.defer();
      require('fs').readFile(file, function(err, data) {
        if (err) {
		require('fs').readFile(path + file, function(err, data) {
			if (err) {
				return deferred.reject(new exports.FileError("while reading " + path + file, null, "cannot read " + file + " (" + err + ")", _this.start_mark));
			} else {
  ...
  */
  ...
  this.RamlParser = (function() {
    ...
    RamlParser.prototype.loadFile = function(file, path, settings) {
      var error,
        _this = this;
      if (settings == null) {
      ...
      try {
        return settings.reader.readFileAsync(file, path).then(function(stream) {
          return _this.load(stream, path, file, settings);
        });
      } catch (_error) {
        error = _error;
    ...
    RamlParser.prototype.load = function(stream, path, location, settings) {
      if (settings == null) {
        settings = this.settings;
      }
    ...
    RamlParser.prototype.parseStream = function(stream, location, settings, parent, path) {
      var loader,
        _this = this;
      if (settings == null) {
        settings = this.settings;
      ...
      return this.q.fcall(function() {
        ...
        return _this.getPendingFiles(loader, partialTree, files, path);
    ...
    RamlParser.prototype.getPendingFiles = function(loader, node, files, path) {
      ...
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        file = files[_i];
        loc.push(this.getPendingFile(loader, file, path).then(function(overwritingnode) {
    ...
    RamlParser.prototype.getPendingFile = function(loader, fileInfo, path) {
      ...
      try {
        if (fileInfo.type === 'fragment') {
          return this.settings.reader.readFileAsync(fileUri, path).then(function(result) {
  ...
  */
  ...
  this.loadFile = function(file, path, settings) {
    var parser;
    if (settings == null) {
      settings = defaultSettings;
    }
    parser = new exports.RamlParser(settings);
    return parser.loadFile(file, path, settings);
  };
  ...
  */
  ...
  this.load = function(stream, path, location, settings) {
    var parser;
    if (settings == null) {
      settings = defaultSettings;
    }
    parser = new exports.RamlParser(settings);
    return parser.load(stream, path, location, settings);
  };
