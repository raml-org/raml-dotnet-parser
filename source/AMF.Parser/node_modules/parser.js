...
let mapModel = (amfModel) => {
    ...
    let mappedModel = {
        ...
        basePath: value(encodes.basePath),
...
let mapSettings = (settings, type) => {
    ...
    return {
        ...
        authorizationUri: value(settings.authorizationUri),
        tokenCredentialsUri: value(settings.tokenCredentialsUri),
        signatures: value(settings.signatures),
        accessTokenUri: value(settings.accessTokenUri),
        authorizationGrants: settings.authorizationGrants,
        flow: value(settings.flow),
        scopes: mapScopes(settings.scopes),
...
let mapShape = (shape) => {
    ...
    return {
        ...
        // array
        minItems: shape.minItems ? shape.minItems.value() : null,
        maxItems: shape.maxItems ? shape.maxItems.value() : null,
        uniqueItems: shape.uniqueItems ? shape.uniqueItems.value() : null,
        items: mapShape(shape.items),
        linkTarget: mapLinkTarget(shape.linkTarget),
        linkLabel: value(shape.linkLabel),

        // scalar
        dataType: shape.dataType ? shape.dataType.value() : null,
        pattern: shape.pattern ? shape.pattern.value() : null,
        minLength: shape.minLength ? shape.minLength.value() : null,
        maxLength: shape.maxLength ? shape.maxLength.value() : null,
        minimum: shape.minimum ? shape.minimum.value() : null,
        maximum: shape.maximum ? shape.maximum.value() : null,
        exclusiveMinimum: shape.exclusiveMinimum ? shape.exclusiveMinimum.value() : null,
        exclusiveMaximum: shape.exclusiveMaximum ? shape.exclusiveMinimum.value() : null,
        format: shape.format ? shape.format.value() : null,
        multipleOf: shape.multipleOf ? shape.multipleOf.value() : null,
        
        // node
        minProperties: shape.minProperties ? shape.minProperties.value() : null,
        maxProperties: shape.maxProperties ? shape.maxProperties.value() : null,
        closed: shape.closed ? shape.closed.value() : null,
        discriminator: shape.discriminator ? shape.discriminator.value() : null,
        discriminatorValue: shape.discriminatorValue ? shape.discriminatorValue.value() : null,
