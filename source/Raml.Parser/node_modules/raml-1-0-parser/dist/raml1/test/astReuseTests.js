...
function test(path1, path2, doReuse) {
    if (doReuse === void 0) { doReuse = true; }
    var path1Res = util.data(path1).replace(/\\/g, '/');
    var path2Res = util.data(path2).replace(/\\/g, '/');
    var api1 = index.loadRAMLSync(path1Res, []).expand();
    ...
    if (doReuse) {
        assert(api2.highLevel().reusedNode() != null, "Reuse is expected");
    }
    else {
        assert(api2.highLevel().reusedNode() == null, "Reuse is not expected");
    }
}
